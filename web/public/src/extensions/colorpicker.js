...
(function(window, document, undefined) {
    ...
    function mousePosition(evt) {
        ...
        if (window.event && window.event.contentOverflow !== undefined) {
            return { x: window.event.offsetX, y: window.event.offsetY, type: evt.type };
        }
        // Webkit:
        if (evt.offsetX !== undefined && evt.offsetY !== undefined) {
            return { x: evt.offsetX, y: evt.offsetY, type: evt.type };
        }
        // Firefox:
        var wrapper = evt.target.parentNode.parentNode;
        return { x: evt.layerX - wrapper.offsetLeft, y: evt.layerY - wrapper.offsetTop, type: evt.type };
    }
    ...
    function slideListener(ctx, slideElement, pickerElement) {
        return function(evt) {
            ...
            ctx.callback && ctx.callback(c.hex, { h: ctx.h - hueOffset, s: ctx.s, v: ctx.v }, { r: c.r, g: c.g, b: c.b }, undefined, mouse, mouse.type);
        }
    };
    ...
    function pickerListener(ctx, pickerElement) {
        return function(evt) {
            ...
            var c = hsv2rgb(ctx);
            ctx.callback && ctx.callback(c.hex, { h: ctx.h - hueOffset, s: ctx.s, v: ctx.v }, { r: c.r, g: c.g, b: c.b }, mouse, undefined, mouse.type);
        }
    };
    ...
    function ColorPicker(slideElement, pickerElement, callback) {
        ...
        if (type == 'SVG') {
            ...
            //null check added to source code by Blake for bug in Safari
            if (hsvGradient == null) {
                hsvGradient = slideClone.getElementsByTagName('linearGradient')[0];
                if (hsvGradient == null) { 
                    console.error('cannot get gradient-hsv element in colorPicker.js');
                }
            }
            ...
            if (blackAndWhiteGradients[0] == null || blackAndWhiteGradients[1] == null) {
                blackAndWhiteGradients = pickerClone.getElementsByTagName('defs')[0].children;
                if (blackAndWhiteGradients[0] == null || blackAndWhiteGradients[1] == null) { 
                    console.error('cannot get gradient-black or gradient-white element in colorPicker.js');
                }
            }
